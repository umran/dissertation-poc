...
class PolicyNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        x = F.relu(self.fc2(x))
        action = 3 * torch.tanh(self.out(x))
        
        return action

def bench_inverted_pendulum():
    random_ddpg = VanillaActorCritic(InvertedPendulum(), DDPG(QNetwork, PolicyNetwork))
    ...
    vanilla_ddpg = VanillaActorCritic(InvertedPendulum(), DDPG(QNetwork, PolicyNetwork))
    ...
    hybrid_hmc = HybridHMC(InvertedPendulum(), DDPG(QNetwork, PolicyNetwork), PolicyNetwork)
