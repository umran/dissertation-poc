...
def bench_pendulum():
    random_ddpg = VanillaActorCritic(Pendulum(), DDPG(QNetwork, PolicyNetwork))
    ...
    vanilla_ddpg = VanillaActorCritic(Pendulum(), DDPG(QNetwork, PolicyNetwork))
    vanilla_ddpg_observer, vanilla_ddpg_results = new_observer(Pendulum())
    vanilla_ddpg.train(observer=vanilla_ddpg_observer)

    hybrid_hmc = HybridHMC(Pendulum(), DDPG(QNetwork, PolicyNetwork), PolicyNetwork)
